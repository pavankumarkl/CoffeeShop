...
class Menu extends Component {
  ...
  componentWillMount() {
    ...
    // )
    console.log(
      "componentWillMount of Menu",
      this.props.addToCartState,
      this.props.addToCartState.value
    );
      ...
      >
        <View style={Styling.billDetails}>
          ...
          <View style={[Styling.billAmount]}>
            <Text style={Styling.amount}>
              {this.state.currency}.{data.price}
            </Text>
          </View>
          {console.log(
            "Cart state in render",
            this.state.cartState[data.id].count
          )}
          <NumericInput
            value={
              this.state.cartState != null
                ? this.state.cartState[data.id].count
                : 0
            }
            // value={0}
      ...
      >
        <ListView
          ...
          renderRow={(rowData, sectionId, rowId) =>
            this.ListRowView(rowData, sectionId, rowId)
          }
        />
      </View>
    );
